...
export interface ProductFormulaEditorHandle {
...
export const ProductFormulaEditor = forwardRef<ProductFormulaEditorHandle, ProductFormulaEditorProps>(
  function ProductFormulaEditor(
    { finishedProductTypeId = '', mode = 'edit', localItems = [], setLocalItems },
    ref
  ) {
    ...
    // Exposed to parent: flushes the pending add-row item if one exists
    useImperativeHandle(ref, () => ({
      flushPendingItem: async () => {
        if (newItem.raw_material_type_id && newItem.quantity_required) {
          await handleAdd();
        }
      },
    }));
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center gap-2 pt-2 border-t">
          ...
          <div className="flex items-center gap-1">
            ...
            />
            {newItem.raw_material_type_id && (() => {
              const unit = rawMaterials.find(t => t.product_type_id === newItem.raw_material_type_id)?.unit;
              return unit ? (
                <span className="text-xs font-mono font-semibold px-1.5 py-0.5 rounded bg-amber-100 text-amber-800 border border-amber-300 whitespace-nowrap">
                  {unit}
                </span>
