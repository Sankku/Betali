...
>
> **Nota de schema**: La tabla `product_formulas` usa `finished_product_type_id` y `raw_material_type_id` (referencias a `product_types`), no directamente a `products`. El PRD original documentó el schema con `products` pero la implementación migró al paradigma `product_types`.
>
> **Frontend** ❌ Pendiente:
