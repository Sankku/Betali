 ...
 *
 * Supabase signs the hook request with a JWT (HS256).
...
function verifyHookJwt(authHeader, hookSecret) {
  ...
  if (!token) {
    logger.warn('Supabase auth hook: no Bearer token in Authorization header', { authHeader: authHeader?.slice(0, 30) });
    return false;
  }
