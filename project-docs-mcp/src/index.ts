...
class ProjectDocsMCP {
  ...
  private tracker: FeaturesTracker;
  private prdGen: PRDGenerator;
  private markdownGen: MarkdownGenerator;

  constructor() {
    this.server = new Server(
      { name: 'project-docs-mcp', version: '1.0.0' }
    );
  ...
  private async detectBugsFromCode(): Promise<string> {
    try {
  ...
  private async updateAllDocumentation(): Promise<string> {
    try {
